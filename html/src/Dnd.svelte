<script>
    ...
    function drop(event, basketIndex){
        ...
        const [item] = baskets[data.basketIndex].items.splice(data.itemIndex, 1)//제거하고 반환
...
{#each baskets as basket, basketIndex}
    <!--{console.log('basket:', basket, ' / basketIndex: ', basketIndex, ' / hoveringOverBasket: ', hoveringOverBasket)}-->
    <b>{basket.name}</b>
